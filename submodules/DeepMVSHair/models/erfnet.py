...
from models.modules import SqueezeExcite


class DownsamplerBlock (nn.Module):
    def __init__(self, ninput, noutput, use_se = False):
        super().__init__()
...
class non_bottleneck_1d (nn.Module):
    def __init__(self, chann, dropprob, dilated, use_se = False):
        super().__init__()
        ...
        self.se = SqueezeExcite(channel=chann) if use_se else None
...
class Net(nn.Module):
    def __init__(self, num_classes, input_channels, encoder=None):  # use encoder to pass pretrained encoder
        ...
        if (encoder == None):
            self.encoder = Encoder(num_classes, input_channels=input_channels)
        else:
